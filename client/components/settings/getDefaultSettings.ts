    ...
    return {
        ...
        stripeVendorId:
            vendors.find((vendor: QBOVendor) => vendor.DisplayName === 'Stripe')
                ?.Id || '',
        stripeFeeAccountId:
            accounts.find((act: QBOAccount) => act.Name === 'Stripe fees')
                ?.Id || '',
        defaultIncomeAccountId:
            accounts.find((act: QBOAccount) => act.Name === 'Stripe sales')
                ?.Id || '',
        defaultTaxCodeId:
            taxCodes.find((taxCode: QBOTaxCode) => taxCode.Name === 'TAX')
                ?.Id || '',
        exemptTaxCodeId:
            taxCodes.find((taxCode: QBOTaxCode) => taxCode.Name === 'NON')
                ?.Id || '',
    };
};
